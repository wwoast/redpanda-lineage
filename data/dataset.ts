...
export class Dataset {
   ...
   * `.txt` INI-format file. The vertex type is made the top-level object
   * key, locations and photos values are unspooled into key/value fields,
   * where each value is a string. These are effectively the opposite of the
   * "processNode" methods.
   */
  renderObject = (entity: GraphNode, edges: Edge[]) => {
    switch (entity.type) {
      case "panda":
        return this.renderPandaObject(entity, edges)
      case "media":
        return this.renderMediaObject(entity)
      case "wild":
      case "zoo":
  ...
  renderMediaObject = (entity: Vertex) => {
    const working = structuredClone(entity)
  ...
  /** TODO: stricter typing on entity, oldnames in PandaNode */
  renderPandaObject = (entity: Vertex, edges: Edge[]) => {
    const working = structuredClone(entity)
